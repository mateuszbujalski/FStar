...
let (check_id : FStar_Ident.ident -> unit) =
  fun id1  ->
    let first_char =
      let uu____5079 = FStar_Ident.text_of_id id1  in
    ...
    then ()
      ...
      (let uu____5087 =
         let uu____5093 =
           let uu____5095 = FStar_Ident.text_of_id id1  in
           FStar_Util.format1
             "Invalid identifer '%s'; expected a symbol that begins with a lower-case character"
             uu____5095
            in
         (FStar_Errors.Fatal_InvalidIdentifier, uu____5093)  in
       let uu____5099 = FStar_Ident.range_of_id id1  in
...
let at_most_one :
  'Auu____5109 .
    Prims.string ->
      FStar_Range.range ->
        'Auu____5109 Prims.list ->
          'Auu____5109 FStar_Pervasives_Native.option
  =
  fun s  ->
    fun r  ->
      fun l  ->
...
let mkWildAdmitMagic :
  'Auu____5882 .
    FStar_Range.range ->
      (pattern * 'Auu____5882 FStar_Pervasives_Native.option * term)
  =
  fun r  ->
...
let focusBranches :
  'Auu____5906 .
    (Prims.bool * (pattern * 'Auu____5906 FStar_Pervasives_Native.option *
      term)) Prims.list ->
      FStar_Range.range ->
        (pattern * 'Auu____5906 FStar_Pervasives_Native.option * term)
          Prims.list
  =
  fun branches  ->
    fun r  ->
...
let focusLetBindings :
  'Auu____6207 .
    (Prims.bool * ('Auu____6207 * term)) Prims.list ->
      FStar_Range.range -> ('Auu____6207 * term) Prims.list
  =
  fun lbs  ->
    fun r  ->
...
let focusAttrLetBindings :
  'Auu____6368 'Auu____6369 .
    ('Auu____6368 * (Prims.bool * ('Auu____6369 * term))) Prims.list ->
      FStar_Range.range -> ('Auu____6368 * ('Auu____6369 * term)) Prims.list
  =
  fun lbs  ->
    fun r  ->
      let should_filter =
...
let (mkTuple : term Prims.list -> FStar_Range.range -> term) =
  fun args  ->
    fun r  ->
      ...
      mkApp (mk_term (Name cons1) r Expr) uu____6733 r
...
let (mkDTuple : term Prims.list -> FStar_Range.range -> term) =
  fun args  ->
    fun r  ->
      ...
      mkApp (mk_term (Name cons1) r Expr) uu____6762 r
...
let to_string_l :
  'Auu____7413 .
    Prims.string ->
      ('Auu____7413 -> Prims.string) ->
        'Auu____7413 Prims.list -> Prims.string
  =
  fun sep  ->
    fun f  ->
      fun l  ->
...
let rec (term_to_string : term -> Prims.string) =
  fun x  ->
    ...
    | Tvar id1 -> FStar_Ident.text_of_id id1
    | Uvar id1 -> FStar_Ident.text_of_id id1
          ...
          to_string_l " "
            (fun uu____7590  ->
               ...
               | (a,imp) ->
                   ...
                   FStar_Util.format2 "%s%s" (imp_to_string imp) uu____7598)
            args
           in
    ...
    | App (t1,t2,imp) ->
        ...
        FStar_Util.format3 "%s %s%s" uu____7618 (imp_to_string imp)
          uu____7621
    ...
    | Bind (id1,t1,t2) ->
        let uu____7877 = FStar_Ident.text_of_id id1  in
    ...
    | CalcProof (rel,init1,steps) ->
        ...
        let uu____8523 = term_to_string init1  in
        let uu____8525 =
...
let lids_of_let :
  'Auu____8814 .
    (pattern * 'Auu____8814) Prims.list -> FStar_Ident.lident Prims.list
  =
  fun defs  ->
...
let (decl_is_val : FStar_Ident.ident -> decl -> Prims.bool) =
  fun id1  ->
    fun decl  ->
      match decl.d with
      | Val (id',uu____9228) -> FStar_Ident.ident_equals id1 id'
      | uu____9229 -> false
