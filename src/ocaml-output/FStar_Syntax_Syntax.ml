      ...
      let aq =
        FStar_List.map
          (fun uu____8295  ->
             ...
             | (bv,t) -> let uu____8306 = f t  in (bv, uu____8306))
          qi.antiquotes
         in
  ...
  fun bv  ->
    fun aq  ->
      let uu____8323 =
        FStar_List.tryFind
          (fun uu____8341  ->
             match uu____8341 with | (bv',uu____8350) -> bv_eq bv bv') aq
         in
...
let syn :
  'Auu____8388 'Auu____8389 'Auu____8390 .
    'Auu____8388 ->
      'Auu____8389 ->
        ('Auu____8389 -> 'Auu____8388 -> 'Auu____8390) -> 'Auu____8390
  = fun p  -> fun k  -> fun f  -> f k p 
let mk_fvs :
  'Auu____8421 .
    unit -> 'Auu____8421 FStar_Pervasives_Native.option FStar_ST.ref
  = fun uu____8430  -> FStar_Util.mk_ref FStar_Pervasives_Native.None 
let mk_uvs :
  'Auu____8438 .
    unit -> 'Auu____8438 FStar_Pervasives_Native.option FStar_ST.ref
...
let (bv_to_tm : bv -> term) =
  fun bv  ->
    let uu____8628 = range_of_bv bv  in
    mk (Tm_bvar bv) FStar_Pervasives_Native.None uu____8628
  
let (bv_to_name : bv -> term) =
  fun bv  ->
    let uu____8635 = range_of_bv bv  in
    mk (Tm_name bv) FStar_Pervasives_Native.None uu____8635
  ...
  fun s  ->
    fun r  ->
      fun t  ->
        ...
        { ppname = id1; index = uu____9392; sort = t }
...
let (freshen_bv : bv -> bv) =
  fun bv  ->
    let uu____9415 = is_null_bv bv  in
    if uu____9415
    then
      let uu____9418 =
        let uu____9421 = range_of_bv bv  in
        FStar_Pervasives_Native.Some uu____9421  in
      new_bv uu____9418 bv.sort
    else
      (let uu___604_9424 = bv  in
...
let (freshen_binder : binder -> binder) =
  fun b  ->
    ...
    | (bv,aq) -> let uu____9440 = freshen_bv bv  in (uu____9440, aq)
...
let (new_univ_name :
  ...
  fun ropt  ->
    ...
    let uu____9455 =
      let uu____9461 =
        let uu____9463 = FStar_Util.string_of_int id1  in
...
let (set_bv_range : bv -> FStar_Range.range -> bv) =
  fun bv  ->
    fun r  ->
      let uu___631_9572 = bv  in
      let uu____9573 = FStar_Ident.set_id_range r bv.ppname  in
      {
        ppname = uu____9573;
...
let (fv_to_tm : fv -> term) =
  fun fv  ->
    let uu____9602 = FStar_Ident.range_of_lid (fv.fv_name).v  in
    mk (Tm_fvar fv) FStar_Pervasives_Native.None uu____9602
...
let (range_of_fv : fv -> FStar_Range.range) =
  fun fv  ->
    let uu____9636 = lid_of_fv fv  in FStar_Ident.range_of_lid uu____9636
  
let (set_range_of_fv : fv -> FStar_Range.range -> fv) =
  fun fv  ->
    fun r  ->
      let uu___644_9648 = fv  in
      let uu____9649 =
        let uu___646_9650 = fv.fv_name  in
        let uu____9651 =
          let uu____9652 = lid_of_fv fv  in
