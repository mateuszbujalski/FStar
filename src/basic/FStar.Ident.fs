...
type lident = {ns:ipath; //["FStar"; "Basic"]
               ...
               str_hash:hash_val; // a hash of 'str', for fast comparison
               str:string} // Cached version of string_of_lid

// IN F*: [@ PpxDerivingYoJson PpxDerivingShow ]
type lid = lident

let mk_ident (text,range) = {idText=text; text_hash=phys_hash text; idRange=range}
...
let lid_of_ns_and_id ns id =
    ...
    let s = (if nsstr="" then id.idText else nsstr ^ "." ^ id.idText) in
    {ns=ns;
     ident=id;
     nsstr=nsstr;
     str_hash=phys_hash s;
     str=s}
let lid_of_ids ids =
...
let lid_equals l1 l2 =
    l1.str_hash = l2.str_hash
    && l1.str = l2.str

let ident_equals id1 id2 =
    id1.text_hash = id2.text_hash
